...
public:
	...
	// Dostęp do transform
	TransformA& GetTransform() { return transform; }
	Physics& GetPhysics() { return physics; }
	Renderable& GetRenderable() { return render; }
...
public:
	...
	void Draw() const {
		if (hasTexture) {
			float scale = 0.2f;
			Vector2 dstPos = {
				transform.position.x - (texture.width * scale) / 2.0f,
				transform.position.y - (texture.height * scale) / 2.0f
			};
	...
	float GetRadius() const {
		return hasTexture ? (texture.width * 0.2f * 0.5f) : (type == WeaponType::BULLET ? 5.f : 2.f);
	}
	...
	Texture2D tex = {})
{
	...
	if (wt == WeaponType::LASER) {
		return Projectile(pos, vel, 20, wt, tex);
	}
	else {
		return Projectile(pos, vel, 10, wt, tex);
	}
}
...
public:
	PlayerShip(int w, int h) : Ship(w, h) {
		texture = LoadTexture("icevan.png");
...
public:
	...
	void Run() {
		...
		Texture2D icecreamTexture = LoadTexture("icecream.png");
		if (icecreamTexture.id == 0) {
			TraceLog(LOG_ERROR, "Nie udalo sie zaladowac tekstury lodu!");
		}
		SetTextureFilter(icecreamTexture, TEXTURE_FILTER_BILINEAR);
			...
			// Shooting
			{
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					while (shotTimer >= interval) {
						...
						Vector2 direction = Vector2Subtract(mousePos, p);
						projectiles.push_back(MakeProjectile(currentWeapon, p, direction, projSpeed, icecreamTexture));
						shotTimer -= interval;
					}
				}
				else {
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end(); ++ait) {
					...
					if (dist < (*pit).GetRadius() + (*ait)->GetRadius()) {
						int currentSize = (*ait)->GetSize();
						if (currentSize > 1) {
							auto pos = (*ait)->GetPosition();
							auto newSize = static_cast<Renderable::Size>(currentSize / 2);

							for (int i = 0; i < 2; ++i) {
								auto newAsteroid = MakeAsteroid(C_WIDTH, C_HEIGHT, currentShape);
								newAsteroid->GetTransform().position = pos;
								newAsteroid->GetRenderable().size = newSize;

								float angle = Utils::RandomFloat(0, 2 * PI);
								newAsteroid->GetPhysics().velocity = Vector2Scale({ cosf(angle), sinf(angle) },
									Utils::RandomFloat(100.f, 200.f));
...
private:
	...
	static constexpr int C_WIDTH = 1900;
	static constexpr int C_HEIGHT = 1030;
